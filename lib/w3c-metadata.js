...
function makeCleaner(status, isRetired, isSuperseded) {
    return function(spec) {
        ...
        var obj = {
            ...
            obsoletes: walk(spec, "doc:obsoletes", 0, "$", "rdf:resource"),
            source: RDF_FILE
        };
...
exports.previousVersion = function(url, publishDate, callback) {
    parseTrRdf(function(err, trdata) {
        ...
        var versions;
        if (trdata[url] !== undefined) {
          versions = trdata[url].versions || undefined;
        }
        if (trdata[url] === undefined || !versions) {
            callback("No previous version found for " + url, null);
            return;
        }
        var dates = Object.keys(versions).sort().reverse();
        if (dates[0] && dates[0] !== publishDate.replace(/-/g, '')) {
        ...
        if (dates.length > 1) {
            ...
            var obsoletes = versions[dates[0]].obsoletes;
            if (obsoletes) {
              var previous = {};
              var pdate = obsoletes.match(/[1-2][0-9]{7}/)[0];
              previous.status = obsoletes.match(/\/TR\/[0-9]{4}\/([A-Z]+)/)[1];
              previous.rawDate = pdate.substring(0, 4) + '-' +
